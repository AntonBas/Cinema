    ...
    return (
        <div className={styles.table}>
            ...
            {sessions.map(session => {
                const occupancy = getOccupancyPercentage(session.ticketsSold, session.hallCapacity);
                ...
                return (
                    <div key={session.id} className={styles.tableRow}>
                        <div className={styles.movieInfo}>
                            ...
                            <div className={styles.movieMeta}>{session.movieDuration} min</div>
                        </div>

                        <div className={styles.hallInfo}>
                            <div className={styles.hallName}>{session.hallName}</div>
                            <div className={styles.capacity}>{session.hallCapacity} seats</div>
                        </div>
                        ...
                        <div className={styles.price}>{formatCurrency(session.basePrice)}</div>
                        ...
                        <div className={styles.actions}>
                            {editable && (
                                <Button variant="success" size="small" onClick={() => onEdit(session)}>
                                    Edit
                                </Button>
                            )}
                            {cancellable && (
                                <Button variant="secondary" size="small" onClick={() => onCancel(session)}>
                                    Cancel
                                </Button>
                            )}
                            {reactivatable && (
                                <Button variant="success" size="small" onClick={() => onReactivate(session)}>
                                    Reactivate
                                </Button>
                            )}
                            {deletable && (
                                <Button variant="error" size="small" onClick={() => onDelete(session)}>
                                    Delete
                                </Button>
                            )}
                        </div>
