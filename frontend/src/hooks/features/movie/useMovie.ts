...
export const useMovie = () => {
  ...
  const loading = useDelayedLoading(
    adminMoviesApi.loading ||
      currentMoviesHomeApi.loading ||
      upcomingMoviesHomeApi.loading ||
      leavingSoonHomeApi.loading ||
      movieDetailApi.loading ||
      adminMovieApi.loading ||
      searchMoviesApi.loading ||
      mutationApi.loading,
    { delay: 150, minDisplayTime: 300 },
  );

  const getAdminMovies = useCallback(
    async (
      params?: SearchParams & { query?: string; status?: MovieStatus },
    ) => {
      return adminMoviesApiRef.current.execute(() =>
        movieApi.admin.getAll(params),
      );
    },
    [],
  );
  ...
  const getCurrentMoviesForHome = useCallback(async () => {
    return currentMoviesHomeApiRef.current.execute(() =>
      movieApi.public.getCurrentMoviesForHome(),
    );
  }, []);

  const getUpcomingMoviesForHome = useCallback(async () => {
    return upcomingMoviesHomeApiRef.current.execute(() =>
      movieApi.public.getUpcomingMoviesForHome(),
    );
  }, []);

  const getLeavingSoonForHome = useCallback(async () => {
    return leavingSoonHomeApiRef.current.execute(() =>
  ...
  const getBySlug = useCallback(async (slug: string) => {
    return movieDetailApiRef.current.execute(() =>
  ...
  const search = useCallback(async (query?: string, date?: string) => {
    return searchMoviesApiRef.current.execute(() =>
      movieApi.public.search(query, date),
    );
  }, []);
  ...
  const update = useCallback(
    async (id: number, request: MovieUpdateRequest) => {
      return mutationApiRef.current.execute(
  ...
  return {
    ...
    homeError:
      currentMoviesHomeApi.error ||
      upcomingMoviesHomeApi.error ||
      leavingSoonHomeApi.error,
