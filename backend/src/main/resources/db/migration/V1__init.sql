...
CREATE TABLE IF NOT EXISTS payments (
    ...
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    user_id BIGINT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    discount_id BIGINT REFERENCES discounts(id) ON DELETE
    SET NULL
);
CREATE TABLE IF NOT EXISTS tickets (
    ...
    payment_id BIGINT REFERENCES payments(id) ON DELETE
    SET NULL,
        discount_id BIGINT REFERENCES discounts(id) ON DELETE
    SET NULL,
        ...
        discount_amount DECIMAL(10, 2),
        promotion_id BIGINT REFERENCES promotions(id) ON DELETE
    SET NULL,
...
CREATE TABLE IF NOT EXISTS bonus_transactions (
    ...
    payment_id BIGINT REFERENCES payments(id) ON DELETE
    SET NULL,
        refund_id BIGINT REFERENCES refunds(id) ON DELETE
    SET NULL,
        type VARCHAR(30) NOT NULL,
...
ADD CONSTRAINT check_percent_range CHECK (
        percent >= 0
        AND percent <= 100
    );
ALTER TABLE movies
ADD CONSTRAINT check_dates CHECK (end_showing_date > release_date);
ALTER TABLE tickets
ADD CONSTRAINT check_prices_positive CHECK (
        (
            calculated_price IS NULL
            OR calculated_price >= 0
        )
        AND (
            final_price IS NULL
            OR final_price >= 0
        ...
        AND (
            base_price_at_purchase IS NULL
            OR base_price_at_purchase >= 0
        )
        AND (
