...
public class BonusRulesInitializer implements ApplicationRunner {
    ...
    private void initializeWelcomeBonusRule() {
        if (bonusRulesRepository.findByBonusType(BonusTransactionType.WELCOME_BONUS).isEmpty()) {
    ...
    private void initializeBirthdayBonusRule() {
        if (bonusRulesRepository.findByBonusType(BonusTransactionType.BIRTHDAY_BONUS).isEmpty()) {
    ...
    private void initializeBookingSpendRule() {
        if (bonusRulesRepository.findByBonusType(BonusTransactionType.BOOKING_SPEND).isEmpty()) {
    ...
    private void initializePaymentAccrualRule() {
        if (bonusRulesRepository.findByBonusType(BonusTransactionType.PAYMENT_ACCRUAL).isEmpty()) {
    ...
    private void initializeRefundReturnRule() {
        if (bonusRulesRepository.findByBonusType(BonusTransactionType.REFUND_RETURN).isEmpty()) {
