...
public interface PersonRepository extends JpaRepository<Person, Long> {
	...
	@Query("SELECT p FROM Person p WHERE " + "(:name IS NULL OR LOWER(p.name) LIKE LOWER(CONCAT('%', :name, '%'))) AND "
			+ "(:role IS NULL OR p.role = :role)")
	Page<Person> findByFilters(@Param("name") String name, @Param("role") PersonRole role, Pageable pageable);
